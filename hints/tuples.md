...
The largest tuple possible in Acadia has three entries. Once you get to four, it is best to make a record with named entries.
...
Now it is clear that the dimensions should be `Float64` values. It is also clear that we are not using the convention of specifying the top-left and bottom-right corners. It could be clearer about whether the `x` and `y` is the point in the top-left or in the middle though!
