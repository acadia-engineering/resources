...
These languages generally agree that an `Int` may be implicitly cast to a `Float` when necessary. So everyone is doing it, why not Acadia?!

## Type Inference + Implicit Casts

Acadia comes from the ML-family of languages. Languages in the ML-family that **never** do implicit casts include:
...
So for any ML-family language designed before Scala, it is safe to assume that implicit conversions just was not an option. Okay, but what about Acadia?! It comes after Scala, so why not do it like them?!
...
Second, Acadia very much favors explicit code, so this also fits in with the overall spirit of the language and libraries.

I hope that clarifies why you have to add those `Int32.toUInt32` functions! It definitely can take some getting used to, but there are tons of folks who get past that acclimation period and really love the tradeoffs!
