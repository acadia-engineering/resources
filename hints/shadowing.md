...
Most linters produce warnings on variable shadowing, so Acadia makes “best practices” the default. Just rename the first one to `maybeName` and move on.
...
Without shadowing, the code easier to read and folks spend less time on pointless debugging. The net outcome is that folks have more time to make something wonderful with Acadia!
